...
class impossivel_adicionar_saldo_negativo_e : public std::exception {
public:
    const char* what() const noexcept override {
        return "Impossível adicionar saldo negativo.";
    }
};

class saldo_insuficiente_para_remocao_e : public std::exception {
public:
    const char* what() const noexcept override {
        return "Saldo insuficiente para remoção.";
    }
};
 ...
 *   1. Garantir que o adicionar saldo lança uma exceção. - FEITO
 *   2. Garantir que o remover saldo lança uma exceção. - FEITO
 *   3. Ajustar bugs e leaks caso existam - FEITO
 */
class Cliente {
    private:
        std::string _login;
